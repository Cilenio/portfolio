...
<body>
    <nav class="navbar ">
        ...
        <a href="registarViatura.php"><button type="button" class="btn btn-danger">Adicionar veiculos </button></a>
    ...
    <div class="tabela"> 
        <div class="login">
            <h3 class="text-center">Registar Viatura</h3>
            <form method="post">
        ...
        <?php
            if(isset($_POST["registar"])){
                ...
                $modelo = $_POST["modelo"];
                if(empty($fabricante) || empty($ano_fabrico) ||empty($matricula) ||empty($propretario_id) ||empty($modelo)){
                    echo" Os Campos email e senha sao obrigatorios";
                }else{
                    $util = new viaturaController();
                    $util->adicionarViatura($fabricante, $ano_fabrico, $matricula,$modelo, $propretario_id);
