...
<head>
    ...
    <title>REGISTAR PROPRETARIO</title>
...
<body>
    <nav class="navbar ">
        <a href='registarProprietario.php'><button type="button" class="btn btn-danger">Adicionar Proprietarios</button></a>
    ...
    <div class="tabela"> 
        <div class="login">
            <h3 class="text-center">Registar Proprietario</h3>
            <form method="post">
...
<?php
    if(isset($_POST["registar"])){
        ...
        $sexo = $_POST["Sexo"];
        if(empty($email) || empty($nome) || empty($sobrenome) || empty($endereco)|| empty($sexo)){
            echo" Os Campos email e senha sao obrigatorios";
        }else{
            $util = new proprietarioController();
            $util->criarProprietario($nome, $sobrenome, $email, $endereco, $sexo);
